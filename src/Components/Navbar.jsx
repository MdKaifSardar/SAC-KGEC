...
const Navbar = ({ sections, activeSection, onNavClick }) => {
  ...
  return (
    <section className="py-5 fixed top-0 p-1 z-50 bg-white pl-10 pr-10 flex flex-row w-full">
      <nav className="flex flex-row justify-between items-center w-full h-30">
        <div className="hover:cursor-pointer gap-5 flex flex-row w-fit h-fit justify-center items-center">
          <Link to="/" className="border-r-2 pr-5 border-black">
            <img
          ...
          <Link to="/" className="text-black sm:text-4xl text-3xl font-sans font-semibold">
            SAC-KGEC
          </Link>
        </div>
      ...
      {/* mobile nav */}
      <div className="z-40 md:hidden flex flex-col justify-center items-center">
        <button
          onClick={() => {
            setIsOpen(!isOpen);
          }}
        ...
        <nav className={`${isOpen ? "navMenu-open" : "navMenu-closed"} h-screen`}>
          <button
